...
class Server {
	...
	private function by_request( $content ) {
		...
		if ( $state_id ) {
			...
			// Also check GET parameters for activityId
			if ( empty( $url ) ) {
				$url = ultc_get_filter_var( 'activityId', '' );
			}
			...
			Database::log( sprintf( 'by_request: url after content/referer=%s', $url ), 'Server' );
			...
			if ( strstr( $url, '/uncanny-snc/' ) === false && function_exists( 'getallheaders' ) ) {
				...
				if ( isset( $all_headers['uotcmodulereferer'] ) ) {
					...
					Database::log( sprintf( 'by_request: url from header=%s', $url ), 'Server' );
				}
			}
