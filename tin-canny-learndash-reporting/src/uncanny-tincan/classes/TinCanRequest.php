...
abstract class TinCanRequest {
	...
	protected function save() {
		...
		if ( ! empty( $activity ) ) {
			...
			$activity_name = isset( $activity_data->_map['und'] ) ? $activity_data->_map['und'] : '';

			foreach ( $lang_keys as $lang_key ) {
				if ( isset( $activity_data->_map[ $lang_key ] ) ) {
					$activity_name = $activity_data->_map[ $lang_key ];
					break;
				}
			}

			// If still empty, try the asVersion method as a fallback
			// Ticket #77236
			if ( empty( $activity_name ) && method_exists( $activity_data, 'asVersion' ) ) {
				$desc_version = $activity_data->asVersion();
				if ( ! empty( $desc_version ) && is_array( $desc_version ) ) {
					// Try to match the preferred language order
					foreach ( $lang_keys as $lang_key ) {
						if ( isset( $desc_version[ $lang_key ] ) ) {
							$activity_name = $desc_version[ $lang_key ];
							break;
						}
					}
					// If no preferred language found, fall back to first available
					if ( empty( $activity_name ) ) {
						$activity_name = reset( $desc_version );
					}
				}
			}
